...
<body>
	...
	%>
	<br>
<b>Details for Auction# <%= request.getParameter("id") %> </b>	

<br>

	<% try { //Get the database connection 
	ApplicationDB db = new ApplicationDB(); Connection con = db.getConnection(); //Create a SQL statement 
	...
	String str1 = "SELECT itemID, username, open_date, close_date, current_price, bid_increment, current_bidder FROM auction WHERE auctionID = '" + num + "'" ;
...
//	ResultSet result = stmt.executeQuery(str); 
	...
	<table>
	<tr>
		<td><input type="radio" id="regular" name="bidtype" value="bid" checked> Bid
		</td>
		<td><input type="radio" id="auto" name="bidtype" value="auto"> Automatic Bidding
		</td>
	</tr>
	
	
	<tr>
		<td> Price/Maximum Price:
		</td>
		<td>
			<input type="number" min="0.00" step="0.01" name="bidprice">
		</td>
	</tr>
	
	<tr>
		<td>Make Bid Anonymous (Hide Username): </td>
		<td><input type="radio" id="notanon" name="isanon" value="notanon" checked> No
		</td>
		<td><input type="radio" id="anon" name="isanon" value="anon"> Yes
		</td>
	</tr>
